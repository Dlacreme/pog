defmodule Pog.Accounts.UserNotifier do
  import Bamboo.Email
  use Bamboo.Phoenix, view: PogWeb.LayoutView
  alias Pog.ActionEmail
  ...
  defp deliver(email) do
    {:ok,
     new_email(
       to: email.to,
       from: Mailer.smtp_email_adress()
     )
     |> put_layout({PogWeb.LayoutView, :email_action})
     |> assign(:email, email)
     |> render(:email_action)
     |> Mailer.inline_css()
     |> Mailer.deliver_now()}
